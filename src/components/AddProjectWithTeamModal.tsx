...
interface AddProjectWithTeamModalProps {
  onClose:   () => void
  onCreated: () => void
}
...
export default function AddProjectWithTeamModal({ onClose, onCreated }: AddProjectWithTeamModalProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!projectRes.ok) {
        const d = await projectRes.json().catch(() => null)
      ...
      // Step 2: Create Team with the new project_id
      ...
      if (!teamRes.ok) {
        const d = await teamRes.json().catch(() => null)
        setError(
          `Project created successfully, but team creation failed: ${d?.error ?? 'Unknown error'}. ` +
          `You can add a team manually from Teams Map using the "+ Add Team" button.`
        )
        return
      }

      // Success: both Project and Team created
      onCreated()
    } catch {
    ...
    >
      <div className="bg-white border border-neutral-200 rounded-2xl w-full max-w-5xl mx-4 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="shrink-0 px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
          <h3 className="text-base font-semibold text-neutral-950">New Project</h3>
