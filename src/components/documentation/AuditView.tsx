...
const EVENT_CONFIG: Record<string, { label: string; dotColor: string; badgeClass: string }> = {
  ...
  unlock:         { label: 'Unlock',               dotColor: 'bg-gray-500',   badgeClass: 'text-gray-400 bg-gray-50 border-gray-200' },
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-gray-800/50">
        {filtered.length === 0 ? (
          ...
          const cfg  = EVENT_CONFIG[e.event_type] ?? { label: e.event_type, dotColor: 'bg-gray-600', badgeClass: 'text-gray-400 bg-gray-50 border-gray-200' }
          const cpId = e.metadata?.checkpoint_id as string | undefined
          const cp   = cpId ? cpMap.get(cpId) : null
          ...
          return (
            <div key={e.id} className="px-6 py-4 hover:bg-white/40 transition-colors">
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  ...
                  {e.workspace_id && (
                    <button onClick={() => router.push(`/workspace/${e.workspace_id}`)}
                      className="text-xs text-gray-500 hover:text-gray-600 transition-colors text-right">
                      Open Document →
                    </button>
                  )}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-4xl mx-4 max-h-[85vh] flex flex-col shadow-2xl">
            <div className="shrink-0 px-6 py-4 border-b border-gray-800 flex items-center justify-between gap-4">
              ...
              <div className="shrink-0 flex items-center gap-2">
                ...
                <button onClick={() => setDetailCpId(null)} className="text-gray-500 hover:text-gray-600 text-sm px-2">✕</button>
              </div>
            </div>
                      ...
                      <div className="p-3 space-y-2 overflow-y-auto max-h-72">
                        {g.messages.map((msg, i) => (
                          <div key={i} className={`text-xs rounded-lg px-3 py-2 leading-relaxed whitespace-pre-wrap ${
                            msg.role === 'user' ? 'bg-indigo-900/50 text-indigo-100 ml-3' : 'bg-gray-100 text-gray-800 mr-3'
                          }`}>
