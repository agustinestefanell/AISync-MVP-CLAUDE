  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-stretch border-b border-[#E2E8F0]">
      ...
      <button
        ...
        className="flex items-center px-1.5 text-[#B7C2D3] hover:text-[#5C6B82] cursor-grab active:cursor-grabbing touch-none"
...
export default function MapView({
  ...
  // Orden manual (drag & drop) de los Projects en el sidebar. projectGroups ya
  // llega ordenado por sort_order (persistido en DB vía el prop `teams`), así
  // que este estado solo existe para reflejar un drag en curso sin esperar un
  // refetch — se resiembra cuando cambia el conjunto de IDs (proyecto nuevo,
  // archivado, etc.) preservando el orden ya elegido para los que persisten.
  const [orderedProjectIds, setOrderedProjectIds] = useState<string[]>([])
  const [reorderError, setReorderError] = useState<string | null>(null)

  useEffect(() => {
    const currentIds = projectGroups.map(g => g.id)
    setOrderedProjectIds(prev => {
      const prevFiltered = prev.filter(id => currentIds.includes(id))
      const newIds = currentIds.filter(id => !prevFiltered.includes(id))
      const next = [...prevFiltered, ...newIds]
      // Evita un setState (y re-render) cuando el orden no cambió realmente.
  ...
  const orderedProjectGroups = useMemo(() => {
    const byId = new Map(projectGroups.map(g => [g.id, g]))
    return orderedProjectIds
      .map(id => byId.get(id))
      .filter((g): g is (typeof projectGroups)[number] => g !== undefined)
  }, [projectGroups, orderedProjectIds])
  ...
  async function persistProjectOrder(ids: string[]) {
    try {
      ...
      if (!res.ok) {
        setReorderError('Could not save the new project order.')
        return
      }
  ...
  const allProjectLayouts = useMemo(() => {
    return orderedProjectGroups.map(project => {
  ...
  }, [orderedProjectGroups, teamCodes, connectionMetadata, connectionStatus])
                ...
                {orderedProjectGroups.map((project) => (
                  <SortableProjectRow key={project.id} id={project.id}>
                    <div
                      ...
                      className="w-full text-left px-2 py-3 hover:bg-[#F8FBFF] transition-colors cursor-pointer"
                    >
