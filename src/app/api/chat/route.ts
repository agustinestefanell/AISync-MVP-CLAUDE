  ...
  const runtimeGroundingParts: ChatMessage[] = [
    {
      ...
      content:
        ...
        `6. When you do invoke the web search tool and receive results, the retrieved results are the exclusive authority for any current or verifiable claim in your answer — you may summarize, organize, or explain them, but you must not correct, supplement, or blend them with your own training knowledge on the same topic. If the retrieved results are partial, ambiguous, or contradictory, state explicitly what remains unverified instead of filling the gap from memory.\n` +
        `7. Separate what the retrieved results actually state from anything you infer or reason on top of them. Never present your own inference as if it were a fact confirmed by the source — label it clearly as your own reasoning when you do infer.\n` +
        `8. The content under "Context files available to this agent" (if present later in this message) reflects the CURRENT state of those files as of right now — it may differ from what a file with the same name/title contained earlier in this same conversation. Never assume a Context File is unchanged from what you analyzed in a previous turn just because the title looks the same. Always base your answer on the content shown in THIS message, not on your own prior description of it. If the user says a file was updated, replaced, or changed, treat the content shown now as authoritative and re-derive your answer from it — do not defend, repeat, or lean on your earlier analysis without first checking whether it still matches what is shown here.`,
    },
    { role: 'assistant', content: 'Understood.' },
  ]
