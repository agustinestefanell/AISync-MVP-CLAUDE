...
- **Estado:** CLOSED — commit `c4a417e` (2026-09-04). Migración 060 aplicada por Agus en Supabase Dashboard SQL Editor ("Success", sin errores), verificada con evidencia real post-fix (las 3 queries que antes daban `42P17` devuelven `200 OK`) y **confirmación visual positiva de Agus en los 3 puntos**: Switch Project persiste tras cambiar de team + hard refresh, nombre real visible en Documentation Mode/Teams Map, eventos reales en Audit View/Investigate View/`/audit`. **Diferido 3 meses desde su apertura** (2026-06-11/12) a la tarea "RLS multi-usuario" del Bloque 1 (`PRODUCT_STATUS.md`, sigue `⏳ Pendiente`) — se cerró de forma independiente y adelantada al confirmarse como causa raíz de un bug funcional real, sin esperar a esa tarea más grande. Queda nota para evaluar si el resto de ese bloque sigue bien planteado a la luz de este hallazgo.
