...
| Attachment transparency text (2026-07-08) | ⚠️ **Partial** | Attachment chips now include a small transparency note below the chip area explaining that file content is analyzed but not stored — only metadata and AI summary are kept. The note appears once per message with attachments (not repeated per chip). No attachment upload, metadata, audit_log or storage behavior was changed. Visual validation pending. |
