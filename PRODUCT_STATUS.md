...
Last updated: 2026-07-23 (Connect Team active Project binding + UI relocation — partial, pending PO validation)
...
| **Connect Team active Project binding + UI relocation** | ⏳ Partial | **2026-07-23** — Migration 050 adds `requester_project_id` and `receiver_project_id` to `team_connections` with ON DELETE SET NULL. POST `/api/connections` populates `requester_project_id` from request body and creates isolated team Host with that `project_id`. PATCH `/api/connections/[id]` (accept) populates `receiver_project_id` from request body and creates isolated team Invitee with that `project_id`. Legacy connections (created before migration) keep NULL in both columns (no backfill). **UI relocation (Frente 2):** Connect button REMOVED from common ribbon in TeamsClient. Connect button ADDED to individual Project headers in MapView. TeamsClient state `connectProjectId` tracks which Project triggered the modal. ConnectTeamModal and IncomingRequestsPanel receive `projectId` prop from parent context (no visible selector). **Validations:** lint ✅, TypeScript ✅, git diff 7 files / +461 -110. **Pending:** Migration 050 applied in Supabase + integral testing (7-point checklist) + screenshot PO confirming Connect relocated from ribbon to Project headers. See handoff-2026-07-b.md 2026-07-23 for full detail. |
