...
| Feature | Estado | Evidencia | Notas |
|---|---|---|---|
| MAP view | ✅ Closed | Commit pending | **v3: Hierarchical org chart (2026-07-14).** Teams Map rebuilt as hierarchical org chart with Project accordion, Executive Team synthetic root (when multiple projects), and algorithmic tree positioning. Canvas with pan (left-click drag) and zoom (wheel, 0.25-2.0 range). **Components:** `buildTreeLayout.ts` (recursive positioning algorithm), `teamsMapLayoutTypes.ts` (LayoutNode/Tree types), `teamsMapLayoutHelpers.ts` (positioning helpers), `v3/` components folder. **MapView.tsx:** Complete v3 rewrite (+640 lines net) — pan/zoom state with useRef, accordion Projects with expand/collapse, TeamCard with top color header + hierarchical codes (A-00/A-01/A-01-01), WorkerBox (max 2 per team), Shared Team with black banner "Shared with [email]", SVG connectors parent→children, Legend with 4 blocks. **TeamsClient:** Eliminated Map/Tree toggle (Map-only), added Project selector in ribbon. **Validated visually in production (2026-07-14, localhost:3000/teams with real data):** 14-point checklist ✅ including hierarchical tree (not bento), accordion functional, Executive Team synthetic, max 2 workers respected, correct hierarchical codes, Shared Team integrated, pan/zoom/Open/Edit/Add Team/Connect/Requests working, SAT/MAT preserved. **Legacy preserved:** CanvasViewport and TreeView.tsx untouched. No changes to modals, API routes, migrations, RLS, schema. |
...
| **Archived Teams — Fase 1A** | ⏳ Partial | commit fc9964b | **Estado estructural base implementado.** Migration 049 agrega columnas: status (active/archived), archived_at, archived_by (UUID), archive_reason (TEXT). types.ts extendido con TeamStatus y 4 campos nuevos en Team interface. API PATCH con action='archive' pobla status/archived_at/archived_by/archive_reason sin borrar workspaces/sessions/chats/checkpoints. EditTeamModal botón "Archive Team" con confirmación double-click + optional reason textarea. **RLS gap oficialmente cerrado:** Policy teams_update confirmada aplicada en Supabase real por PO (no solo archivo repo). audit_log.metadata confirmado existente como JSONB — NO requiere migración para Fase 1C. **Fase 1A scope:** Solo estado estructural. NO Restore/Unarchive, NO Teams Map UX, NO audit_log events (diferidos a fases 1B/1C). Validaciones: lint ✅, build ✅, grep Restore/audit_log: 0 results ✅. **Pending:** Migration 049 aplicada en Supabase + validación funcional PO (13-point checklist). |
| **Archived Teams — Fase 1B** | ✅ Closed | Commit pending | **Teams Map visibility implementado y validado.** Archived teams ocultos por defecto (`showArchived=false`). Control secundario `Show archived (N)` / `Hide archived` visible solo cuando `archivedCount > 0`. Ubicado en ribbon operativo después de contador Teams/Workers. Filtrado respeta jerarquía parent-child (`filterArchivedTeams()`) — parent archived oculto significa hijos también ocultos (previene nodos huérfanos). Archived visibles se muestran en posición normal del árbol (no sección separada). Badge `Archived` (amber #FEF3C7) coexiste con SAT badge sin overlap (positioning dinámico `top: isSat ? '48px' : '12px'`). **Ajuste visual:** Archived teams visibles muestran `opacity: 0.45` (40-50% range) en card completa para diferenciación marcada. **Workers heredan opacity:** Worker cards de team archivado heredan `opacity: 0.45` del padre via `isWorkerArchived = realTeam?.status === 'archived'`. **Bug fix crítico:** `realTeam` era `undefined` para Workers porque buscaba por `node.id` (sintético) en lugar de `node.teamId` (real) — corregido. Badge "Archived" preserva `opacity: 1` para legibilidad. Open/Edit funcionales (opacity NO afecta pointer-events). Colores de team preservados intactos. Conteo N sobre Project seleccionado (no hay modo All Projects). **Archivos:** TeamsClient.tsx (+20 líneas: estado + control + conteo), MapView.tsx (+40 líneas: filtrado + prop isArchived + Worker inheritance), TreeWorkspaceCard.tsx (+20 líneas: badge Archived + opacity 0.45). **Validaciones:** lint ✅, build ✅, grep Restore/Unarchive: 0 ✅, CanvasViewport/TreeView NO modificados ✅. **Validado visualmente PO (2026-07-16):** Managers/Workers archivados opacity 0.45 consistente, badge legible, Open/Edit clickeable. NO Restore/Unarchive. NO Audit Log events. NO snapshots (diferidos Fase 1C). |
| **Teams Map — Project headers outside zoom** | ✅ Closed | Commit ad24847 | **Project section headers now render outside the zoom/pan transform layer (2026-07-22).** Each Project header (name + Teams counter) moved from inside single CanvasViewport to individual stable position before each Project's CanvasViewport. Structure changed: CanvasViewport moved from wrapping all Projects (single) to individual per Project. Headers maintain stable font size and full visible-width (`w-full`) alignment regardless of zoom level. Pan horizontal does not move headers. Zoom/pan signals shared across all CanvasViewports for synchronized behavior. Gap-12 between Projects preserved. Stack vertical layout intact. **Files:** MapView.tsx (+14 lines net: CanvasViewport per-project, header w-full). **Validations:** lint ✅, build ✅, no TreeView/accordion/Map-Tree toggle reintroduced ✅. **Validated by PO (2026-07-22):** Headers remain legible in zoom-out, no horizontal displacement with pan, all Teams Map v3 features intact. |
| **Teams Map — Project Index Sidebar** | ✅ Closed | Commit pending | **Collapsible sidebar for quick navigation between Projects (2026-07-23).** Fixed left sidebar (280px width) with list of all Projects, each showing name + "N teams" counter. Click on Project scrolls smoothly to that section (`scrollIntoView({ behavior: 'smooth' })`). Sidebar default closed; vertical tab "PROJECTS" opens it. Close button (✕) in sidebar header. Sidebar stays open after navigation for consecutive jumps. Paleta Dashboard redesign (#ffffff bg, #DDE6F1 border, #0C1733 text, #F8FBFF hover). No active state (users can see multiple Projects in zoom-out). Fixed positioning z-30, no conflict with canvas zoom/pan. **Files:** MapView.tsx (+79 lines net: sidebar + tab + scrollToProject). **Validations:** Validated visually by PO before commit — tab opens sidebar ✅, list shows Projects ✅, scroll suave ✅, sidebar stays open ✅, close button ✅, no conflict with zoom/pan ✅. No changes to TeamsClient, CanvasViewport v3, TreeView, or legacy components. |
| **Loading feedback en botones críticos** | ✅ Closed | Commit 2026-07-22 | **Critical async action buttons now provide loading feedback.** Replicated ApiKeyRequiredModal pattern across 5 components: EditTeamModal (Archive/Save), ConnectTeamModal (Connect), AgentPanel (Send), HumanChatPanel (Send), AddTeamModal (Create). Changes: (1) EditTeamModal `handleArchive()` refactored to use `try/finally`, Archive button shows `"Archiving..."` when `confirmingArchive && saving`, added `disabled:cursor-not-allowed` to Archive and Save buttons. (2) ConnectTeamModal added `disabled:cursor-not-allowed` to Connect button (already had text/finally/disabled). (3) AgentPanel changed Send text from `'…'` to `'Sending...'`, added `disabled:cursor-not-allowed`. (4) HumanChatPanel changed Send text from `'…'` to `'Sending...'` (already had cursor-not-allowed). (5) AddTeamModal added `disabled:cursor-not-allowed` to Create button (already had text/finally/disabled). **Validated by PO (2026-07-22):** Archiving.../Sending... work correctly, buttons return to normal state after async completion. Pattern: loading state + disabled during process + dynamic descriptive text + guaranteed restoration with `finally` + consistent disabled classes (`opacity-50`, `cursor-not-allowed`). |
| **UX polish: Archive modal, Worker Open, SAT default** | ✅ Closed | Commit 2026-07-22 | **Three UX refinements after PO visual validation.** (1) EditTeamModal Save button hidden during Archive confirmation (`{!confirmingArchive && (...)}`) — button out of context during Archive flow. (2) TreeWorkspaceCard primary action button hidden when `actionLabel` empty or undefined (`actionLabel && actionLabel.length > 0` check before render) — Workers with empty actionLabel no longer show non-functional Open button. **Opción A approved by PO:** Fix in TreeWorkspaceCard (generic component) rather than MapView (specific usage) — cleaner, affects all cards with empty actionLabel. (3) AddTeamModal `teamMode` default changed from `'MAT'` to `'SAT'` — SAT is most common case (80%+), reduces unnecessary clicks. **Validated by PO (2026-07-22):** Save disappears/reappears with Archive confirmation, Worker Open button hidden, Manager/Submanager Open visible, SAT preselected when opening Add Team modal. No changes to Archive logic, SAT/MAT logic, APIs, schema, RLS, or migrations. |
...
| **API Keys visual guides** | ✅ Closed | commit 62dc148 | **Mini-OE 2026-07-24:** 3 screenshots con flechas numeradas integrados en SetupGuide.tsx. Imágenes reales provistas por PO (1280×720 PNG, ~335KB total): anthropic-api-key-steps.png, openai-api-key-steps.png, google-api-key-steps.png en `public/setup-guide/`. Next.js Image component con dimensiones explícitas (width={1280} height={720}), responsive (w-full h-auto), border sutil. Imágenes complementan pasos textuales (no reemplazan). Renderizado condicional: solo providers cloud (Anthropic/OpenAI/Google), Local AI sin imagen. Validaciones: lint ✅, build ✅, validación visual PO ✅. Ver handoff-2026-07-b.md 2026-07-24. |
| **API Keys UX refinements** | ✅ Closed | commit pending | **Mini-OE 2026-07-24:** 6 ajustes UX post-validación visual. (1) Modal persistente: removido `hasChecked` state — modal reaparece en cada navegación mientras no haya keys. (2) Resaltado header guía: colores oficiales `--color-accent` (#1f4e79) + texto blanco, consistente con "+ New Project". (3) Botón guía en modal: link prominente "Quick setup guide →" abre `/settings` en nueva pestaña. (4) Imágenes clickeables: wrapper `<a target="_blank">` + hint text "Click to open image in new tab". (5) Contraste advertencia: `text-yellow-300` (WCAG AAA vs anterior amber-600/80). (6) Regresión corregida: fondo azul movido de contenedor exterior a `<button>` trigger — body de guía mantiene `bg-white/40` original. Validaciones: lint ✅, regresión corregida ✅, validación visual PO ✅. Ver handoff-2026-07-b.md 2026-07-24. |
...
| **Groq functional provider removal (2026-07-10 + 2026-07-29)** | ✅ Closed | Commits dc6d3de, pending 2026-07-29 | **Mini-OE A (2026-07-10):** Core functional support removed — `src/lib/providers/groq.ts` deleted, GroqProvider removed from factory registry, 'Groq' removed from KNOWN_PROVIDERS, groqProvider removed from chat API (tool loop + direct stream), Groq removed from onboarding/ApiKeyRequiredModal. **Mini-OE B (2026-07-29):** Cosmetic cleanup completed — Groq removed from SMPanel.tsx PROVIDER_MODELS, AgentCard.tsx/TeamNode.tsx PROVIDER_COLOR, AgentPanel.tsx attachment warning, TokenUsageBadge.tsx provider mapping, TeamsClient.tsx help text. **'Groq' PRESERVED in RESERVED** (providers/route.ts) to prevent custom providers with name "Groq". **Active functional providers:** Anthropic, OpenAI, Google, IA Local. **Legacy data:** 21 agent_sessions migrated to OpenAI GPT-5.5 on 2026-07-10 (commit 9581871). EditTeamModal fallback legacy handles any residual Groq agents as "(legacy)" without allowing new selection. **Validations (2026-07-29):** lint ✅, Supabase query confirmed 0 Groq providers in production ✅. Ver handoff-2026-07-b.md 2026-07-29. |
...
| **Markdown rendering in chat messages (2026-07-11 + 2026-07-29)** | ✅ Closed | Commits de21877, pending 2026-07-29 | **AgentPanel and HumanChatPanel now render Markdown.** Installed react-markdown@^10.1.0 + remark-gfm@^4.0.1. AgentPanel (line 676) and HumanChatPanel (line 495) now use ReactMarkdown with explicit Tailwind components for: p, strong, em, ul, ol, li, table, thead, th, td, code (inline/block), blockquote. **Security:** NO rehype-raw, NO dangerouslySetInnerHTML — safe for Connected Teams content from other accounts. **Copy preserved:** copyMessage still copies original msg.content, not rendered HTML. **Bundle impact:** /workspace/[id] First Load JS increased from 20.1 kB to 63.8 kB (+43.7 kB). **Validations:** lint ✅, build ✅, grep rehype-raw/dangerouslySetInnerHTML: 0 results ✅. **Validated visually in production (2026-07-11):** Comparative table, bold text, and list rendered correctly in AgentPanel. **Documentation Mode fix (2026-07-29):** RepositoryView.tsx and AuditView.tsx now render Markdown correctly in (1) panel detalle conversación, (2) cards preview lista, (3) mensajes expandidos AuditView. Preview compacto usa inline-only rendering con placeholders `[table]`/`[code block]` para compatibilidad con `line-clamp-3`. Validado visualmente por PO en documento "Tabla de m2/dormitorio". Ver handoff-2026-07-b.md 2026-07-29. |
| **Workspace performance — memo + virtualización (2026-07-30)** | ✅ Closed | Commit pending | **Causa raíz de la lentitud progresiva del Workspace:** input y lista completa de mensajes en el mismo componente sin memoización — cada tecla y cada chunk de streaming re-parseaba TODO el Markdown histórico (panel Manager BARRIO HIPICO: 26 msgs / 173KB → ~395ms de CPU por tecla). **Fix en 3 capas:** (1) MessageBubble/HumanMessageBubble memoizados — Markdown se parsea una sola vez; (2) React.memo en AgentPanel/HumanChatPanel + props estabilizadas en WorkspaceShell (panelBindings useMemo) — un panel ya no propaga renders a los hermanos; (3) virtualización con react-virtuoso@4.18.11 — solo mensajes visibles montados en DOM. **Métricas:** Markdown por tecla ~395ms → ~0ms; mount ~395ms → ~65ms; DOM 26 → ~8-12 mensajes. **Tradeoff conocido:** selección nativa de texto (drag) limitada a la ventana visible; Save Selection/Forward no afectados (estado React). Validación PO en producción pendiente. Ver handoff-2026-07-b.md 2026-07-30. |
| **Save Selection reset + copy button sticky (2026-07-30)** | ✅ Closed | Commit pending | **Mini-OE, 2 ajustes UX:** (1) Save Selection ahora limpia los checkboxes de todos los paneles tras guardado exitoso (mismo patrón que R&F/Handoff) — antes la selección quedaba oculta pero activa y se colaba en la siguiente acción. Incluye fix de fallo silencioso: el modal chequea `res.ok` — si el guardado falla, queda abierto con error visible y la selección intacta. (2) Botón de copiar mensaje ahora `position: sticky` — permanece visible al scrollear dentro de respuestas largas, compatible con react-virtuoso, con fondo `bg-white/85` para legibilidad sobre texto. Validación visual PO en producción pendiente. Ver handoff-2026-07-b.md Mini-OE 2026-07-30. |
| **Save as Excel / Save as Word (2026-07-30)** | ✅ Closed | Commit pending | **Fase 1 de export a archivos:** 2 botones en el modal de Save Selection que exportan los mensajes seleccionados a .xlsx/.docx descargables. Server-side: `POST /api/export/excel` y `/api/export/word` (auth sesión, 0 B en bundle cliente). **Excel:** hoja "Messages" (fila por mensaje) + cada tabla Markdown extraída a hoja propia con celdas reales (parser completo, sin fallback — 10 tablas del mensaje real de BARRIO HIPICO verificadas). **Word:** headings, bullets, negrita inline y tablas reales, fidelidad básica. **Deps:** xlsx@0.20.3 desde CDN oficial SheetJS (npm tiene 2 vulns altas sin fix — ver AUDIT_REPORT DEP-001, aprobado por PO) + docx@9.7.1. Helper nuevo `src/lib/export/markdown.ts` (stripMarkdown() existente trunca/colapsa — inviable para export). No guarda en DB. **✅ Validado por PO en producción 2026-07-30:** tablas en hojas separadas y formato Word funcionando. **Ajuste post-validación:** al completarse la descarga, el modal se cierra y la selección se limpia (helper compartido `finishSelectionAction()` con Save Selection; fallo mantiene modal + selección). Ver handoff-2026-07-b.md 2026-07-30. |
| **File size limits — mensaje claro en vez de "Server error" (2026-07-31)** | ⏳ Deployed — PO validation pending | Commit pending | **Límite duro de Vercel (4.5 MB/request, HTTP 413) ahora comunicado claramente.** Validación client-side ANTES de subir: 4 MB en Context Files (FormData binario) / 3 MB en adjuntos del chat (base64 infla ~33% — 4 MB de archivo produce ~5.3 MB de payload). Mensaje muestra nombre, tamaño real y límite. Red de seguridad: catch específico del status 413 en ContextFilePanel, AgentPanel y SMPanel. Helper centralizado `src/lib/upload/limits.ts`. Vercel Blob (eliminaría el límite) diferido a proyecto aparte por decisión de producto. Ver handoff-2026-07-b.md 2026-07-31. |
| **Word/Excel attachments — análisis uniforme pre-provider (2026-07-31)** | ⏳ Deployed — PO validation pending | Commit pending | **Word (.docx) y Excel (.xlsx/.xls) ahora se adjuntan Y se analizan en chat y Context Files.** Ningún provider acepta Office nativo (Anthropic 400, OpenAI falla silencioso, Gemini degrada) — confirmado por PO con los 3. Solución del lado AISync: `src/lib/chat/inlineAttachments.ts` convierte adjuntos Office a texto extraído inline en /api/chat ANTES de la bifurcación por provider (uniforme para los 3). Excel parseado con librería `xlsx` ya instalada (hoja → CSV con nombre); tope 150K chars por adjunto en chat. Imágenes/PDF siguen nativos. **Limitaciones honestas:** PPT/PPTX y .doc legacy se adjuntan/guardan SIN análisis (nota al usuario y al modelo) — PPT diferido a fase siguiente por PO (jszip ya investigado y prototipado, presente como dep transitiva de docx). **Actualización mismo día:** PPTX implementado — ver fila siguiente. Ver handoff-2026-07-b.md 2026-07-31. |
| **PPTX attachments — extracción de texto real (2026-07-31)** | ⏳ Deployed — PO validation pending | Commit pending | **PPTX ahora se adjunta Y se analiza en chat y Context Files.** Rama nueva en extractText.ts: jszip 3.10.1 (ya presente como dep de docx, declarada directa sin descarga nueva) + extracción propia de tags `<a:t>` por slide, formato `[Slide N]`. Candidatas descartadas: node-pptx-parser (2 dep trees nuevos, poco mantenida), office-text-extractor (duplicaría mammoth/pdf-parse/xlsx). El chat no requirió cambios (inlineAttachments ya delega en extractTextFromBuffer). Límites 3MB/4MB aplican automáticos. **`.ppt` legacy binario NO soportado** (sin librería JS confiable — nota honesta, igual que .doc). Smoke test 5/5. Ver handoff-2026-07-b.md OE PPTX 2026-07-31. |
...
| **Connect Team complete chain: Project binding + Duplicate prevention + UI polish** | ✅ Closed | **2026-07-23** — Complete Connect Team feature closed with 5 frentes: (1) Teams Map layout without accordion, (2) Project binding Host (Connect button in Project headers), (3) Duplicate prevention (1 per pair, max 2 total) + connection limits, (4) Disconnected team opacity (0.40, Manager only), (5) Project binding Invitee (explicit selector). **Migration 050:** `requester_project_id` / `receiver_project_id` added to `team_connections`. **Invitee selector:** IncomingRequestsPanel shows Project dropdown when `projects.length > 1`, auto-selects if 1 Project. **Sync fix:** `router.refresh()` client-side + `revalidatePath('/teams')` server-side resolves race condition — isolated teams appear immediately after accept. **Dashboard polish:** Connected Teams cards show "Your Project: [name]" derived from `requester_project_id` / `receiver_project_id` by role. **Opacity refinement:** Disconnected 0.40 (Manager only, NOT Workers), Archived unchanged 0.45 (Manager + Workers). **Disconnected opacity propagation committed 2026-07-24** (delayed verification). **Validations:** lint ✅, build ✅, 7-point PO checklist ✅. Files: +62 lines across 6 files. See handoff-2026-07-b.md 2026-07-23/2026-07-24 for complete detail. |
