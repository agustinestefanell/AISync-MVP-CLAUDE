...
| **MODEL_MAP / Provider routing (2026-07-10)** | ✅ Closed | Pending commit | **Anthropic MODEL_MAP:** Sonnet legacy labels (Claude Sonnet, Claude 3.5 Sonnet, Claude 3.7 Sonnet) now redirect to `claude-sonnet-4-6` (updated from `claude-sonnet-4-5`). New label added: Claude Sonnet 4.6. Claude 3 Haiku and Claude 3 Opus remain unchanged. **OpenAI MODEL_MAP:** New label added: GPT-5.5 → `gpt-5.5`. Existing OpenAI labels (GPT-4o, GPT-4o Mini, GPT-4 Turbo, o1, o3 Mini) remain unchanged to preserve behavior of existing sessions. **No labels deleted** — preserves compatibility with sessions that have legacy labels persisted in `agent_sessions.model`. Google and Groq were not changed. Add/Edit Team modal options remain pending for Subtarea 2. |
