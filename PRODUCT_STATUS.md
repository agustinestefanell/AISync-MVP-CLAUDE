...
| **ConnectTeamModal isolated team exclusion** | ⚠️ Partial | **2026-07-06** — ConnectTeamModal now excludes isolated/shared teams from the automatic host team selection used to start a new connection. Filters `teams.filter(t => t.type !== 'isolated')` before selecting hostTeamId. This prevents new connections from being created from an already shared team and avoids names like "Shared: Shared: ...". If no eligible team available, shows error: "No eligible team available to connect from. Isolated (shared) teams cannot be used to start new connections." Existing corrupted legacy team names are not corrected retroactively. Only ConnectTeamModal.tsx modified (+5 lines). No changes to accept backend, Teams Map, RLS, or migrations. **Status Partial:** Código y build confirmados, pero comportamiento real no probado — 5 de 7 casos de validación funcional sin confirmar en vivo (incluyendo el caso central: que una conexión nueva ya no genere "Shared: Shared:"). Pendiente validación visual del Product Owner. |
