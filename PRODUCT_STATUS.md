...
| Feature | Estado | Evidencia | Notas |
|---|---|---|---|
| MAP view | ⏳ Partial | Pending commit | **Draft 2 literal reconstruction applied (v2).** Teams Map rebuilt to literally match approved Draft 2 design. Project layout uses bento/masonry-like column flow (`columns-1 xl:columns-2`) instead of uniform grid. Project names resolve from real project data; PROJECT UNKNOWN bug fixed. Team Cards use top color header with white code/name text (not left border). Team Card body remains white with provider/model/SAT-MAT text badge/compact metrics WS:N SES:N WRK:N/Open-Edit actions. Subteams render directly below their parent team with visual connectors (vertical border-left + horizontal line per subteam). Subteams use lighter shades derived from parent team color (`deriveLighterColor 25%`). Legend added with 4 approved explanatory blocks (Project=Container, Team=Color, Subteam=Lighter Shade, Workspace/Sessions=Compact Metadata) using exact literal text. Map remains only visible Teams Map view. TreeView preserved/deprecated. CanvasViewport untouched. Helper `assignTeamColor.ts` provides deterministic fallback when team.color is null. **Validations:** lint ✅, build ✅, TypeScript ✅, grep PROJECT UNKNOWN: 0 ✅, grep CanvasViewport MapView: 0 ✅. **Pending:** PO screenshot validation of Draft 2 literal layout (top color header, bento mosaic, horizontal connectors, project real name, legend visible). |
