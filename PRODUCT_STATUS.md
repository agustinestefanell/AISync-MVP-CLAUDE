...
| Teams Map — Ribbon buttons + SAT/MAT + Create Teams modals | ✅ Closed | commit feat: connect teams map ribbon buttons and add sat mat and create teams modals | Tres botones del ribbon interno conectados. Modales `Single Agent Team (SAT) and Multiple Agent Team (MAT)` y `How to create or grow Teams` agregados con copy aprobado completo. Labels corregidos: "How to create or grow Teams" y "SAT vs MAT: How they work and how to use them →". Link de Col 2 centrado en ribbon con `absolute left-1/2 -translate-x-1/2`. |
