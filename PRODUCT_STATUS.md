...
| Feature | Estado | Evidencia | Notas |
|---|---|---|---|
| MAP view | ⏳ Partial | Pending commit | **Complete UI redesign from approved design assets.** Teams Map rebuilt as responsive CSS grid layout grouped by project. Previous canvas/pan/zoom interaction removed from active UI. Map is now the only visible Teams Map view. Projects render as visual containers with aggregated metrics (total teams/sessions/workers). Teams render as cards with provider/model/SAT-MAT badge/workspaces-sessions-workers counters. Subteams render nested under parent with lighter derived color tone. Team color is visual identity (4px left border); SAT/MAT remains text badge. Existing Open/Edit/Add/Connect flows preserved. Helper `deriveTeamColor.ts` created for color derivation and fallback. **Validations:** lint ✅, build ✅, TypeScript ✅. **Pending:** PO screenshot validation of project grid with multiple teams, visible colors, and subteam with derived tone. |
