...
| Feature | Status | Notes |
|---|---|---|
| `session_attachments` table | Partial | Migración 021 aplicada. `chat/route.ts` registra eventos fire-and-forget en `session_attachments` y `audit_log` (`attachment_uploaded`). Audit Log UI muestra badge "File Attached" + filename. Chat muestra chip de adjunto en historial de mensajes. Validación runtime pendiente. |
