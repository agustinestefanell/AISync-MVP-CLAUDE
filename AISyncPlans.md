...
**Teams Map Draft 2 visual contract (2026-07-12 v2):** Active Teams Map uses bento/masonry-like Project layout via CSS columns (`columns-1 xl:columns-2` with `columnGap: 16px`). Project containers use `break-inside-avoid` to prevent mid-container breaks. Project names resolve from real `projectName` prop (never PROJECT UNKNOWN). Project containers show aggregated metrics: Teams (main teams count), Subteams (total subteams count), Sessions, Workers. Teams flow inside each Project using `flex flex-wrap`. **Team Card visual contract:** top color header (backgroundColor = team color) with white code/name text, white body with provider/model (bold/normal text), SAT/MAT text badge only (not color-encoded), compact metrics (WS:N SES:N WRK:N), Open/Edit actions. **Subteams visual contract:** render under specific parent team (not grouped at project bottom), use lighter shade derived from parent team color (`deriveLighterColor(parentColor, 0.25)`), visual connector parent→subteam via vertical `border-l-2` + horizontal `h-px w-3` line per subteam, smaller card without provider/model/SAT-MAT (only code/name/metrics/Open-Edit). **Legend:** 4 exact blocks visible (Project=Container, Team=Color, Subteam=Lighter Shade, Workspace/Sessions=Compact Metadata) with literal approved text. **Color resolution:** `assignTeamColor.ts` helper provides `resolveTeamColor(team)` — uses `team.color` if present, else deterministic fallback from 11-color palette by hash of `team.id`. Map remains only visible Teams Map view. TreeView preserved/deprecated. CanvasViewport untouched. Future cleanup may remove orphan CanvasViewport and TreeView only after explicit OE.
