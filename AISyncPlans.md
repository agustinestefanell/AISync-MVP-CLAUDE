...
**Excepción — Teams Map**: `TeamsClient` gestiona layout completo (TopRibbon + ribbon operativo interno + BottomRibbon). El ribbon interno tiene dos botones de ayuda: `How to create or grow Teams` → `showCreateTeamsGuide` (Col 2) y `SAT vs MAT: How they work and how to use them →` → `showSatMatGuide` (Col 3). El subtítulo del `TopRibbon` superior abre `showMainGuide`. No duplicar "How to use Teams Map" en el ribbon interno. `page.tsx` retorna `<TeamsClient pageName projectName .../>` directamente.
