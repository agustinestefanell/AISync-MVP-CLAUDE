  ...
  └── div.flex-1
      ...
      └── Vista activa
            ...
            library     → UserLibraryView (2026-08-21, reemplaza a Structure View)
                            └── LoadAsContextButton (2026-08-26, por card de Save Selection)
            audit       → AuditView
...
**User Library (2026-08-21) — reemplaza a Structure View como tab.** Basada ÚNICAMENTE en Save Selection, organizada por tags manuales libres (no carpetas anidadas) — schema nuevo `tags`/`saved_selection_tags` (migración 058, ver sección 5.1). Un Save Selection puede tener varios tags. "Estar en la library" se deriva de tener ≥1 fila en `saved_selection_tags` — sin columna `in_user_library` en `saved_selections` (ver DECISIONS.md 2026-08-21): el modal de Save Selection exige elegir/crear al menos 1 tag cuando el usuario tilda "Add to User Library", así que ese estado intermedio (checkbox tildado, 0 tags) no existe en la práctica. `UserLibraryView.tsx`: panel izquierdo con buscador de tags (client-side, "contains", 2026-08-21) + lista A-Z con contador de Save Selections por tag (`useMemo` sobre `savedSelections[].tags`, sin query aparte) y selección múltiple (checkboxes, no exclusiva) — panel derecho en grid responsive (`grid-cols-1 sm:grid-cols-2 lg:grid-cols-4`, ajustado a 4 columnas 2026-08-22). Sin ningún tag marcado se ven TODOS los Save Selections de la library; marcar tags suma restricciones (AND — debe tener todos los marcados, no alguno). Estado vacío (ningún Save Selection con al menos 1 tag todavía) es bilingüe (inglés primero, español después) — mismo texto exacto también usado en el ícono "i" de la vista (`USER_LIBRARY_GUIDE`, ver arriba). El modal de Save Selection (`WorkspaceShell.tsx`) ganó: toggle "Add to User Library" (estilo card, no checkbox chico — peso visual pedido explícitamente), selector multi-tag (carga lazy vía `GET /api/tags` al activarlo por primera vez) y "+ Create new tag" inline (`POST /api/tags`, sin salir del modal). `POST /api/save-selection` acepta `tagIds?: string[]` opcional, inserta en `saved_selection_tags` después de crear la fila en `saved_selections` (fail-open respecto al Save Selection ya creado si la asociación de tags falla).

**User Library — edición inline (2026-08-22).** Cada card gana: botón "Expand" (⤢) que abre un modal con el contenido completo del Save Selection (mismo patrón visual que `EditTeamModal.tsx` — overlay + header/body scrolleable/footer), trayendo los mensajes vía el `GET /api/documentation/selection/[id]` ya existente; chips de tags con botón "×" individual ("Remove tag", `DELETE .../tags` con `tagId`) + botón "+ Add tag" con popover (tags existentes como chips clickeables de aplicación inmediata, más "+ Create new tag" inline — mismo look que el selector del modal de Save Selection, pero sin paso de "confirmar": cada click es su propia acción porque el Save Selection ya existe); botón "Delete" con confirmación de 2 clicks (mismo patrón `confirming`/"Confirm deletion?" de `EditTeamModal.tsx`) que saca TODOS los tags del Save Selection (`DELETE .../tags` sin `tagId`) — el objeto Save Selection en sí NO se borra, sigue existiendo fuera de User Library. El panel izquierdo de tags gana "+ New" (crear tag directo desde el panel, reusa `POST /api/tags`) y un ícono "✎ Edit Tag" por fila que despliega name input + 8 swatches de color fijos (`TAG_COLORS` en `UserLibraryView.tsx`) → `PATCH /api/tags/[id]`. Tags ganan columna `color` (migración 059, nullable, default `#1f4e79` = mismo azul de `--color-accent`) — se muestra como punto de color en el panel izquierdo y en los chips de las cards. Todas las mutaciones llaman al endpoint y después `router.refresh()` (mismo patrón ya usado por `TeamsClient.tsx`/`EditTeamModal.tsx`) — sin duplicar el estado del servidor en local state del componente.
...
**`LoadAsContextButton` (componente compartido, `src/components/documentation/LoadAsContextButton.tsx`, 2026-08-26) — "Load as Context" en Repository View (3 detail panels), Audit View (`AuditDetailPanel`), Investigate View (`InvestigationScanPanel`) y User Library (cards de Save Selection).** Alcance: solo destino "→ Context Files" (sin "→ Chat" — requeriría selector de Workspace/sesión aparte, no construido). Al hacer click abre un popover (mismo patrón visual que "+ Add tag" de `UserLibraryView.tsx`) con selector de destino real: dropdown Project (todos los del usuario) + dropdown Team dependiente del Project elegido, **excluyendo siempre `type === 'isolated'`** (Connected/Shared Teams — riesgo de seguridad cross-cuenta). Arranca preseleccionado en el Project/Team de origen del ancla (`originProjectId`/`originTeamId`), pero el usuario puede cambiar a cualquier otro Project/Team propio antes de confirmar. Recién al apretar "Confirm" se hace el fetch del contenido (vía `evidenceUrl`) + `POST /api/context` con el Team/Project realmente elegidos — `scope` siempre `'team'` (el selector obliga a elegir un Team). Confirmación temporal inline ("Loaded to Context Files — Team: X, Project: Y", auto-oculta a los 6s) con el destino real usado, no el de origen. **Conocida como corrección de una primera versión** que resolvía el destino automáticamente sin preguntar — no era lo pedido.

**Knowledge Map — fuera de alcance para "Load as Context" (2026-08-26).** Confirmado por código: es un grafo ReactFlow puro sin ningún mecanismo de selección de nodo ni panel de detalle (`KnowledgeMap.tsx`, sin `onNodeClick`) — construirlo sería una OE aparte, más grande que las otras 4 vistas juntas. Señalado, no implementado.

**Panel derecho siempre visible — Audit View e Investigate View (2026-08-26).** Replican ahora el patrón exacto de Repository View: contenedor izquierdo `${selectedItem ? 'w-1/2' : 'flex-1'}` + `border-r` incondicional, y un bloque `!selectedItem && <div className="hidden md:flex flex-1 items-center justify-center ...">Select a document to view details</div>` cuando no hay selección — antes el panel derecho simplemente no existía sin selección (la lista ocupaba el ancho completo sin placeholder).

**Resaltado de ítem seleccionado con relleno de fondo (2026-08-26).** Repository View, Audit View e Investigate View pasan de borde-only (`border-indigo-500 ring-1 ring-indigo-500`, sin fondo) a `bg-indigo-50 border-indigo-400 ring-1 ring-indigo-200` — confirmado por código que Repository View tampoco usaba relleno antes de este fix; las 3 vistas quedan con el mismo tratamiento nuevo, no una imitando a una preexistente.

**Fix — contenido guardado se mostraba en Markdown crudo (2026-08-26).** Nuevo módulo compartido `src/lib/markdown/documentMarkdown.tsx` (`DOCUMENT_MARKDOWN_REMARK_PLUGINS`/`DOCUMENT_MARKDOWN_COMPONENTS`), portado tal cual desde la config de `react-markdown`/`remark-gfm` que ya usa el chat en vivo (`AgentPanel.tsx`) — `AgentPanel.tsx`/`HumanChatPanel.tsx` mantienen sus propias copias locales sin cambios, no se tocó código de chat en vivo. Aplicado en los 4 lugares que muestran contenido completo/sin truncar de un objeto guardado: `ExpandContentModal.tsx`, `InvestigationScanPanel.tsx` (bloque "Forwarded content"), `AuditDetailPanel.tsx` (mismo bloque), y `src/components/audit/AuditTimeline.tsx` (2 bloques — Audit Log **global** `/audit`, distinto de Audit View de Documentation Mode). `UserLibraryView.tsx` migrado a usar `ExpandContentModal` en vez de su copia inline (ya no duplica el patrón). El excerpt truncado a 140 caracteres del timeline "How this was produced" (`AuditDetailPanel.tsx`) usa `stripMarkdown()` en vez del renderer completo — es un preview corto, no contenido completo, y renderizar Markdown cortado a mitad de sintaxis podía verse mal. `SMPanel.tsx` (chat propio del SM) también muestra texto plano pero queda fuera de alcance — no es contenido guardado reabierto.

**Teams Map — `TreeWorkspaceCard` real vive en `src/components/teams/v3/TreeWorkspaceCard.tsx`, NO en `src/components/teams/map/TeamAgentCard.tsx`.** Este último tiene una copia casi idéntica del mismo componente pero es código muerto (sin ningún import real en `src/`, confirmado por grep) — no confundir los dos si se vuelve a tocar el título/estilo de las cards de Teams Map. Cadena real: `TeamsClient.tsx` → `MapView.tsx` (dynamic import) → `TreeWorkspaceCard` (`./v3/TreeWorkspaceCard`). El canvas (`v3/CanvasViewport.tsx`) aplica `transform: scale(zoom)` con zoom inicial auto-calculado (`Math.min(initialZoom, availableWidth/contentWidth, availableHeight/contentHeight) * 0.92`) para encajar el árbol completo — cualquier ajuste de tamaño de fuente en las cards se percibe escalado hacia abajo en pantalla real, no 1:1 con el valor CSS.

**Preview de tabla colapsado — `stripMarkdown()` (2026-08-26).** `collapseMarkdownTables()` (nueva, en `src/lib/text/stripMarkdown.ts`) detecta un bloque de tabla GFM completo (header + separador `---` + N filas de datos) y lo colapsa a `"[Table: N rows]"` en vez de reemplazar cada fila por separado — corre como paso previo en `stripMarkdown()`/`stripMarkdownPreserveParagraphs()`, antes del resto de los `.replace()` en cadena.

**User Library — preview corto pasa a Markdown real, acotado a esta vista (2026-08-26).** `DocSavedSelection.content_preview_full` (que aplicaba `stripMarkdownPreserveParagraphs()`) reemplazado por `content_raw` — mismo contenido de origen, ahora SIN limpiar, cap de seguridad en 20000 chars. `TruncatedPreview` (`UserLibraryView.tsx`) renderiza ese campo vía `ReactMarkdown` (mismo módulo `documentMarkdown.tsx`) en vez de texto plano — el truncado por altura real (`scrollHeight`/`useLayoutEffect`) es agnóstico al tipo de contenido, no requirió cambios. **Repository View y `LoadContextModal.tsx` siguen usando `content_preview` (stripeado, 200 chars) sin cambios** — decisión explícita: ese campo también sirve de corpus de búsqueda en `LoadContextModal`, y está acotado a 200 chars desde la OE de payload optimization (2026-07-30, `44f0315`, ~95% de reducción) — mandar contenido crudo ahí revertiría esa optimización. `stripMarkdownPreserveParagraphs()` sigue exportada en `stripMarkdown.ts` sin ningún caller activo (no se borró, pedido explícito).
...
| `tags` | Tags manuales libres del usuario — base de User Library (`UNIQUE account_id,name`, migración 058, 2026-08-21). `color` TEXT nullable, default `#1f4e79` (migración 059, 2026-08-22 — "Edit Tag") |
...
| POST/DELETE | `/api/documentation/selection/[id]/tags` | saved_selection_tags | Session — edición de tags por card en User Library (2026-08-22): POST agrega tags a un Save Selection ya existente ("Add Tag", upsert con `ignoreDuplicates`); DELETE sin `tagId` saca TODOS los tags ("Delete" de la card — solo saca del library, no borra el Save Selection), con `tagId` saca solo ese uno ("Remove tag"). Ownership check explícito (patrón SEC-008) antes de mutar |
...
| 059 | `059_tags_color.sql` | tags: color TEXT nullable, default `#1f4e79` — soporte para "Edit Tag" en User Library (ajustes de layout/edición, 2026-08-22) |
...
**Registrado:** 2026-09-14
**Estado:** 🟡 Migración y backend correctos y aplicados desde 2026-09-15 (41 proyectos, sin NULL, sin duplicados por cuenta). **Bug real de frontend encontrado y corregido 2026-09-16** — el cierre del 09-15 ("falsa alarma por timing") fue incorrecto; `TeamsClient.tsx:458` le pasaba a `MapView` un array pre-ordenado por código de team (`A-00`/`B-00`, reflejando `created_at`) en vez del array real ordenado por `sort_order`. Fix de 1 línea + limpieza de código muerto, pusheado a `main`. Detalle: `handoff-2026-07-c.md` OE 2026-09-16, `DECISIONS.md` 2026-09-16. **Verificación visual final en producción pendiente de Agus.**
