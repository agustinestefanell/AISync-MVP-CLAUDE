...
**Excepción — Teams Map**: `TeamsClient` gestiona layout completo (TopRibbon + ribbon operativo interno + BottomRibbon). El ribbon interno tiene tres botones de ayuda conectados: `How to use Teams Map` → `showMainGuide`, `How to create Teams` → `showCreateTeamsGuide`, `Differences and uses` → `showSatMatGuide`. El subtítulo del `TopRibbon` superior también abre `showMainGuide`. `page.tsx` retorna `<TeamsClient pageName projectName .../>` directamente.
