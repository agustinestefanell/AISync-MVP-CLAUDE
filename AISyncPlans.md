...
**Teams Map architecture (2026-07-12):** Active Teams Map layout is now a responsive CSS grid grouped by project (`grid-cols-1 xl:grid-cols-2 auto-rows-min`). CanvasViewport is no longer invoked by MapView. Pan/zoom controls are no longer part of active Teams Map UX. Map is the only visible Teams Map view — the Map/Tree toggle was removed from the ribbon. TreeView.tsx is preserved but deprecated (import commented with deprecation note in TeamsClient). Teams are grouped by `project_id` from `workspace.teams.project_id`. Subteams are derived from `parent_id` in a flat teams list (only 1 level deep, not recursive). `team.color` is the primary visual identity (4px left border on team cards, 3px on subteam cards). Subteam color is derived as a lighter tone of parent team color via `deriveLighterColor(parentColor, 0.4)` helper. If `team.color === null`, a deterministic fallback color is used via `getFallbackTeamColor(team.id)` from 8-color palette. SAT/MAT is represented as a text badge (teal for SAT, purple for MAT), not as color. Provider/model are derived from manager session (`workspaces[0].agent_sessions.find(s => s.agent_role === 'manager')`). Counters: team (workspaces/sessions/workers), project (totalTeams/totalSessions/totalWorkers). Future cleanup may remove orphan CanvasViewport and TreeView only after explicit OE.
